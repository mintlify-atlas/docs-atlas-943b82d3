...
<Tabs>
  <Tab title="Nous Portal">
    ...
    Or use an API key — set `NOUS_API_KEY` in `~/.hermes/.env`. Set provider in config:
...
<Tabs>
  ...
  <Tab title="Memory">
    ...
    memory:
      ...
      memory_char_limit: 2200   # ~800 tokens
      user_char_limit: 1375     # ~500 tokens
      nudge_interval: 10        # Remind agent every N turns
      flush_min_turns: 6        # Save memories before exit after N turns

    # Messaging platform session resets
    session_reset:
      mode: both           # both | idle | daily | none
      idle_minutes: 1440   # 24 hours
      at_hour: 4           # Daily reset at 4 AM
    ```
  </Tab>
</Tabs>
