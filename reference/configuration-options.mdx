...
<ParamField path="display.skin" type="string" default="default">
  Visual theme for the CLI. Built-in skins: `default` (gold/kawaii), `ares` (crimson/bronze), `mono` (grayscale), `slate` (blue developer). Drop custom YAML skins in `~/.hermes/skins/<name>.yaml`.
</ParamField>
